...
def load_team_salaries(TEAM_ID):
  if TEAM_ID == "CHA":
    # Problem with Charlotte Hornets salary page
    TEAM_ID = "CHO"
  if TEAM_ID == "NJN":
    # Problem with Brooklyn Nets salary page
    TEAM_ID = "BRK"
  if TEAM_ID == "NOH":
    # Problem with New Orleans Pelicans salary page
    TEAM_ID = "NOP"
...
def main(OUTPUT_FILE, TEAM_IDS):
  ...
  for team in TEAM_IDS:
    ...
    if headers == None: 
      headers = data[0]
      headers.extend(["salary", "WS%", "expected_salary", "salary_diff"])
  # Export to CSV
  ...
  with open(OUTPUT_FILE,'wb') as csvfile:
    ...
    for player in total_players:
      # Add dollars signs and round monetary figures
      player["salary"]          = "$" + str( round( float ( player["salary"] )          , 2 ) )
      player["expected_salary"] = "$" + str( round( float ( player["expected_salary"] ) , 2 ) )
      player["salary_diff"]     = "$" + str( round( float ( player["salary_diff"] )     , 2 ) )
      # Add keys to list
      player_list = []
      for key in headers:
